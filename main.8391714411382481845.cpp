...
void make_chaos_with_balls ()
{
    int BOTTOM_LANE;
    if (WORLDHEIGHT > 3){ //                   
        BOTTOM_LANE = (rand() % (WORLDHEIGHT-3))+1;
    }
    else{
        BOTTOM_LANE = 1;
    }
    ...
    for (int lane = WORLDHEIGHT-2; lane >= BOTTOM_LANE; lane--)
    {
        int NO_OF_BALLS;
        if (WORLDWIDTH > 3){
            NO_OF_BALLS = (rand () % (WORLDWIDTH-3))+1 ;
        }
        else{
            NO_OF_BALLS = 1;
        }
        //                                                               
        //                   
        place_balls (WORLDWIDTH-NO_OF_BALLS-1,lane,NO_OF_BALLS-1,true) ;
    }
}
...
void make_block ()
{
    const int LEFT   = rand () % (WORLDWIDTH  -4) + 2 ;
    const int BOTTOM = rand () % (WORLDHEIGHT - 4) + 2 ;
    const int WIDTH  = rand () % (WORLDWIDTH  - 4) + 1 ;
    const int HEIGHT = rand () % (WORLDHEIGHT - 4) + 1 ;
    //                                                                       
    place_rectangle (LEFT, BOTTOM, WIDTH-1, HEIGHT-1) ;
...
{
    while (not (in_front_of_wall() and north())){
        if (on_ball()){
            get_ball(); //                                             
        }
        if (in_front_of_wall()){
...
{
    //                   
    while (not on_ball() and not in_front_of_wall()){
        step();
    }
    turn_right();
    move_to_wall();
